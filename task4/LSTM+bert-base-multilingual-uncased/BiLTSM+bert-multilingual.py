...
# from load_data_task4 import load_train_data, load_dev_data, load_test_data  # 前面写的导入数据函数
...
task_output_file = "./task4_result/Bi-lstm_bert_multilabel_result.csv"
checkpoint_save_path = "./Model/muti_labels.h5"
...
def create_model():
    ...
    dropout_hidden = tf.keras.layers.Dropout(0.5)(hidden_states)
    ...
    # print(output.shape)  #(None, 180)

    output = LSTM_layer1(dropout_hidden)
    print(tf.shape(output))

    dropout_output = tf.keras.layers.Dropout(0.5)(output)

    # dense = tf.keras.layers.Dense(256, activation='relu')(dropout_output)
    # dropout = tf.keras.layers.Dropout(0.2)(dense)
    output = tf.keras.layers.Dense(15, kernel_initializer=initializer, activation='sigmoid')(dropout_output)
...
cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_save_path, monitor='val_f1', mode='max', verbose=1, save_weights_only=True, save_best_only=True, )
tb_callback = tf.keras.callbacks.TensorBoard(log_dir='./log/Bilstm_bert_multilingual_Crossentropy_logs', profile_batch=0)
...
history = model.fit(train_dataset.shuffle(1000).batch(8), validation_data=val_dataset.batch(8), epochs=1000,
                    ...
                    callbacks=[Metrics(valid_data=(val_dataset.batch(8), dev_labels)),
                               cp_callback,
                               tb_callback,
                              #  tf.keras.callbacks.EarlyStopping(monitor='val_loss', mode='min', patience=40, verbose=1)
                               ]
                    )
