...
def f1(y_pred,y_true):
    ...
    TP += ((y_pred_t == 1) & (true_labels == 1)).cpu().sum()
    # TN predict 和 label 同时为0
    TN += ((y_pred_t == 0) & (true_labels == 0)).cpu().sum()
    # FN predict 0 label 1
    FN += ((y_pred_t == 0) & (true_labels == 1)).cpu().sum()
    # FP predict 1 label 0
    FP += ((y_pred_t == 1) & (true_labels == 0)).cpu().sum()
...
# epochs为训练轮数 默认值设为1
def train_eval(model, criterion, optimizer, train_loader, val_loader, epochs=50):
    print('loading model...')
