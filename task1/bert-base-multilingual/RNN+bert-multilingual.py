...
class MyModel(nn.Module):
    ...
    def __init__(self, freeze_bert=False, model_name=model_name, hidden_size=768, num_classes=2):
        ...
        # self.lstm = nn.LSTM(hidden_size*4, hidden_size, 4 ,dropout=0.3)
    ...
    def forward(self, input_ids, attn_masks, token_type_ids):
        ...
        # print(out.shape)
        out = nn.ReLU()(out) # print(out.shape)   #[bs, seq_len, hidden_dim*4]--32 192 1536
...
# epochs为训练轮数 默认值设为1
def train_eval(model, criterion, optimizer, train_loader, val_loader, epochs=150):
    print('loading model...')
...
def test(model, dataloader, with_labels=False):
    ...
    pd.DataFrame({'id': ids_test, 'is_humor': pred_label}).to_csv(
...
if __name__ == '__main__':
    ...
    # ## 训练及验证
    # train_eval(model, criterion, optimizer, train_loader, val_loader, epochs=500)  # 训练轮数
