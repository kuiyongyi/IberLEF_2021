...
classes2idx = {'absurd': 0, 'analogy': 1, 'embarrassment': 2, 'exaggeration': 3, 'insults': 4, 'irony': 5,
               'misunderstanding': 6, 'parody': 7, 'reference': 8, 'stereotype': 9, 'unmasking': 10, 'wordplay': 11}
idx2classes = {0: 'absurd', 1: 'analogy', 2: 'embarrassment', 3: 'exaggeration', 4: 'insults', 5: 'irony',
               6: 'misunderstanding', 7: 'parody', 8: 'reference', 9: 'stereotype', 10: 'unmasking', 11: 'wordplay'}
...
predict_class_id_path = './task3_result/predict_class_id.csv'
result_path = './task3_result/task3_classify_result.csv'
...
class CustomDataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.with_labels:  # 如果数据集有标签，则为True
            label = self.data.loc[index, 'humor_mechanism']
...
#         # print(outputs[0].shape) #0 [32, 192, 768])  1 [32, 768]

#         first_hidden_states=outputs[1]  #32 768
#         logits = self.fc(first_hidden_states)  #32 12
#         # logits = tf.keras.layers.Dense(12, activation='softmax')(logit)
...
def process_data(filename, classes2idx, with_labels=True):
    ...
    if with_labels:
        data = data.replace({'humor_mechanism': classes2idx})  # 如果 数据有标签 则将标签 转换为 数字
    return data
...
def eval(model, optimizer, validation_dataloader):
    ...
    print("Validation Accuracy: {}  F1_Macro: {}  ".format(eval_accuracy / nb_eval_steps,eval_f1_ma / nb_eval_steps))  # 精度
...
def test(model, dataloader, with_labels=False):
    ...
    # 将预测结果 写回pred文件中
    pd.DataFrame(data=pred_label, index=range(len(pred_label))).to_csv(predict_class_id_path,header=['humor_mechanism'], encoding='utf-8')
    ...
    with open(predict_class_id_path, encoding='utf-8') as f:
        ...
        # data = data.replace({'humor_mechanism': idx2classes})
        # print(data)
        print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
        ...
        # 将预测结果 写回t文件中
        # pand.DataFrame({'id': ids_test, 'text': text_test, 'humor_mechanism': predic_col}).to_csv(result_path,index=False)
        pand.DataFrame({'id': ids_test, 'humor_mechanism': predic_col}).to_csv(result_path, index=False)
...
if __name__ == '__main__':
    ...
    train_loader = Data.DataLoader(train_set, batch_size=batch_size, num_workers=10, shuffle=True)
    print('------------------------------------------')
    ...
    ## 训练及验证
    train_eval(model, criterion, optimizer, train_loader, val_loader, epochs=3)  # 训练轮数
