...
task_output_file = "./task3_result/bert_multi_label_result.csv"
...
class Metrics(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # upper, lower = 1, 0
        # val_predict[val_predict == val_predict[np.argmax(val_predict, -1)]] = upper
        # print(val_predict[np.argmax(val_predict, -1)])
        # val_predict[val_predict != val_predict[np.argmax(val_predict, -1)]] = lower
...
def create_model():
    ...
    output = tf.keras.layers.Dense(12, kernel_initializer=initializer, activation='sigmoid')(dropout_hidden)
